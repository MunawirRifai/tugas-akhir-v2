...
class AuthService {
  ...
  static Future<Map<String, dynamic>> verifyOtp({
    ...
    return verifyEmail(email: email, otp: otp);
  }
  ...
  static String? extractAccessToken(Map<String, dynamic> response) {
    ...
    final Object? userTokenValue = _firstAvailableValue(user, [
      'access_token',
      'accessToken',
      'token',
      'jwt',
  ...
  static String messageOf(
    ...
    final Object? messageValue = _firstAvailableValue(response, [
      'message',
      'error',
      'detail',
    ]);
    ...
    final Object? nestedMessageValue = _firstAvailableValue(data, [
      'message',
      'error',
      'detail',
    ]);

    final String nestedMessage = nestedMessageValue?.toString().trim() ?? '';
  ...
  static List<Map<String, dynamic>> listOf(Object? value) {
    if (value is List) {
      return value.map(mapOf).where((item) => item.isNotEmpty).toList();
    }

    if (value is Map) {
      final Map<String, dynamic> mappedValue = mapOf(value);

      for (final String key in ['data', 'items', 'users', 'results']) {
  ...
  static String _safeFileName(String rawName, {required String fallback}) {
